...
def get_edit_kb():
    ...
    return types.InlineKeyboardMarkup(inline_keyboard=[[k] for k in kb])

async def clear_full_chat(chat_id):
    async for msg in bot.get_updates():
        try:
...
async def show_client_card(chat_id, client, state=None):
...
async def step_3(message: types.Message, state: FSMContext):
    ...
    await state.update_data(new_client=client)
    await state.set_state(AddClient.step_6_console)
    await message.answer("Шаг 4\nВыберите консоль:", reply_markup=get_console_kb())

@dp.message(AddClient.step_6_console)
async def step_6_console(message: types.Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await cancel_handler(message, state)
        return
    client = (await state.get_data())["new_client"]
...
async def step_7(message: types.Message, state: FSMContext):
    ...
    if message.text.lower() == "нет":
        ...
        client["games"] = []
        await state.update_data(new_client=client)
        await state.set_state(AddClient.step_8)
        await message.answer("Шаг 8\nЕсть ли резервные коды?", reply_markup=get_yesno_kb())
    elif message.text.lower() == "да":
        await state.set_state(AddClient.step_7_games)
        await message.answer("Напиши список игр (каждая с новой строки):", reply_markup=get_cancel_kb())

@dp.message(AddClient.step_7_games)
async def step_7_games(message: types.Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await cancel_handler(message, state)
        return
...
@dp.message(AddClient.step_8)
async def step_8(message: types.Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await cancel_handler(message, state)
        return
    if message.text.lower() == "нет":
        ...
        await state.set_state(AddClient.step_8_photo)
        await message.answer("Загрузите скриншот с резервными кодами (одно фото):", reply_markup=get_cancel_kb())

@dp.message(AddClient.step_8_photo, F.photo)
async def step_8_photo(message: types.Message, state: FSMContext):
...
@dp.message(AddClient.step_8_photo)
async def step_8_photo_text(message: types.Message, state: FSMContext):
    if message.text == "❌ Отмена":
        await cancel_handler(message, state)
    else:
...
async def edit_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == "edit_number":
        ...
        await bot.send_message(callback.message.chat.id, "Измените подписку:", reply_markup=get_subscription_type_kb())
