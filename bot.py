...
DB_FILE = "/data/clients_db.json"  # Путь /data для Railway volume
KEY_FILE = "secret.key"
...
@dp.message(AddEditClient.reserve_photo, ContentTypeFilter(content_types=[types.ContentType.PHOTO]))
...
@dp.message(AddEditClient.edit_reserve, ContentTypeFilter(content_types=[types.ContentType.PHOTO]))
async def edit_reserve_photo(message: types.Message, state: FSMContext):
    ...
    for i, c in enumerate(clients):
        if c["id"] == cid:
            ...
            await state.clear()
            await clear_chat(message)
            text, _ = format_card(c, show_photo_id=True)
            await message.answer_photo(photo_id, caption=text, reply_markup=edit_keyboard(c))
            return

@dp.message(AddEditClient.edit_input)
async def edit_input_handler(message: types.Message, state: FSMContext):
    # Используется для редактирования contact, birth_date, account, console, region
    if message.text == "❌ Отмена":
        await state.clear()
        await clear_chat(message)
        await start_cmd(message, state)
...
async def edit_subs_total_handler(message: types.Message, state: FSMContext):
    ...
    if message.text == "Нет подписки":
        ...
        return
    if message.text == "Одна":
        await message.answer("Выберите подписку (Одна):", reply_markup=ReplyKeyboardMarkup(
            keyboard=[
                [KeyboardButton(text="PS Plus Deluxe"), KeyboardButton(text="PS Plus Extra")],
                [KeyboardButton(text="PS Plus Essential"), KeyboardButton(text="EA Play")],
                [KeyboardButton(text="❌ Отмена")]
...
async def edit_sub_1_start_handler(message: types.Message, state: FSMContext):
    ...
    if subs_total == 2:
        ...
        # Формируем и сохраняем подписку
        await save_edit_subscriptions(message, state)
...
async def save_edit_subscriptions(message, state):
    data = await state.get_data()
    cid = data.get("edit_id")
    clients = load_db()
    for i, c in enumerate(clients):
        if c["id"] == cid:
            ...
            if subs_total == 1:
                sub = {
                    ...
                    "start": data.get("edit_sub_1_start"),
                    "end": calc_end_date(data.get("edit_sub_1_start"), data.get("edit_sub_1_duration"))
                }
                c["subscriptions"] = [sub]
            elif subs_total == 2:
                sub1 = {
                ...
                sub2 = {
                    ...
                    "start": data.get("edit_sub_2_start"),
                    "end": calc_end_date(data.get("edit_sub_2_start"), data.get("edit_sub_2_duration"))
                }
...
def calc_end_date(start_str, duration_str):
    try:
        ...
        months = int(duration_str.replace("м", ""))
        year = start.year + (start.month - 1 + months) // 12
        month = (start.month - 1 + months) % 12 + 1
        day = start.day
        end = start.replace(year=year, month=month, day=day)
...
async def base_start(message: types.Message, state: FSMContext):
    await state.clear()
    await clear_chat(message)
    await message.answer("Раздел База:", reply_markup=base_menu())
...
async def base_export(message: types.Message, state: FSMContext):
    clients = load_db()
    if not clients:
...
async def base_subs_soon(message: types.Message, state: FSMContext):
    clients = load_db()
    soon = []
    now = datetime.now()
    ...
    for c in clients:
        for sub in c.get("subscriptions", []):
            try:
                ...
                continue
    if not soon:
        await message.answer("Подписки не заканчиваются в ближайшие 7 дней.")
        return
    text = ""
    for c in soon:
        c_text, _ = format_card(c)
...
async def base_birthday_soon(message: types.Message, state: FSMContext):
    clients = load_db()
    soon = []
    now = datetime.now()
    limit = now + timedelta(days=7)
    for c in clients:
        bd_str = c.get("birth_date", "отсутствует")
        if bd_str == "отсутствует":
            continue
        try:
            bd = datetime.strptime(bd_str, "%d.%m.%Y")
            this_year_bd = bd.replace(year=now.year)
            if now <= this_year_bd <= limit:
                soon.append(c)
        except:
            continue
    if not soon:
        await message.answer("В ближайшие 7 дней дней рождения нет.")
        return
    text = ""
    for c in soon:
        c_text, _ = format_card(c)
...
async def base_no_sub(message: types.Message, state: FSMContext):
    clients = load_db()
    no_subs = []
    for c in clients:
        ...
        if not subs or (len(subs) == 1 and subs[0].get("name") == "отсутствует"):
            no_subs.append(c)
    if not no_subs:
        await message.answer("Все клиенты имеют подписки.")
        return
    text = ""
    for c in no_subs:
        c_text, _ = format_card(c)
...
async def base_backup(message: types.Message, state: FSMContext):
    if not os.path.exists(DB_FILE):
...
async def base_clear_ask(message: types.Message, state: FSMContext):
    keyboard = ReplyKeyboardMarkup(
        keyboard=[
            [KeyboardButton(text="Да"), KeyboardButton(text="Нет")]
        ], resize_keyboard=True)
    await message.answer("Вы уверены, что хотите очистить базу?", reply_markup=keyboard)
...
async def base_clear_confirm(message: types.Message, state: FSMContext):
    if message.text == "Да":
        ...
        await message.answer("Отмена очистки базы.")
        await state.clear()
        await clear_chat(message)
        await start_cmd(message, state)
        return
...
async def show_stats(message: types.Message, state: FSMContext):
    ...
    n_no_subs = sum(1 for c in clients if not c.get("subscriptions") or (len(c.get("subscriptions")) == 1 and c.get("subscriptions")[0].get("name") == "отсутствует"))
