...
def get_identifier(message):
    ...
    client_data[message.chat.id]["method"] = message.text
...
def ask_birth_option(message):
    remember_message(message)
    client_data[message.chat.id]["username"] = message.text.strip()
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
...
def ask_birth_date(message):
    ...
    if message.text == "Есть":
        ...
        client_data[message.chat.id]["birth_date"] = "отсутствует"
...
def collect_birth_date(message):
    ...
    try:
        ...
        client_data[message.chat.id]["birth_date"] = message.text.strip()
    except:
        client_data[message.chat.id]["birth_date"] = "отсутствует"
    ask_account_info(message)
...
def ask_console(message):
    ...
    client_data[message.chat.id]["email"] = email
    client_data[message.chat.id]["password_raw"] = password
    client_data[message.chat.id]["mail_password"] = mail_pass
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
...
def ask_reserve_code(message):
    ...
    if message.text == "Отмена":
        full_clear(message.chat.id)
        return bot.send_message(message.chat.id, "Добавление отменено.", reply_markup=main_keyboard())
    client_data[message.chat.id]["region"] = message.text
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
...
def process_reserve_code(message):
    ...
    if message.text == "Да":
        ...
        client_data[message.chat.id]["reserve_photo"] = None
...
def save_reserve_photo(message):
    ...
    client_data[message.chat.id]["reserve_photo"] = file_id
    ask_subscription_status(message)
...
def ask_subscriptions_count(message):
    ...
    if message.text == "Отмена":
        ...
        return bot.send_message(message.chat.id, "Добавление отменено.", reply_markup=main_keyboard())
    if message.text == "Нет":
        client_data[message.chat.id]["subscription_name"] = "не оформлена"
        client_data[message.chat.id]["subscription_start"] = ""
        client_data[message.chat.id]["subscription_end"] = ""
...
def choose_first_subscription(message):
    ...
    if message.text == "Отмена":
        full_clear(message.chat.id)
        return bot.send_message(message.chat.id, "Добавление отменено.", reply_markup=main_keyboard())
    client_data[message.chat.id]["subs_total"] = message.text
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
...
def collect_first_subscription(message):
    remember_message(message)
    client_data[message.chat.id]["sub1_type"] = message.text
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
...
def collect_first_duration(message):
    ...
    if message.text == "Отмена":
        full_clear(message.chat.id)
        return bot.send_message(message.chat.id, "Добавление отменено.", reply_markup=main_keyboard())
    client_data[message.chat.id]["sub1_duration"] = message.text
    msg = bot.send_message(message.chat.id, "Дата оформления подписки (дд.мм.гггг):")
    remember_message(msg)
    if client_data[message.chat.id]["subs_total"] == "Одна":
...
def calculate_subscriptions_single(message):
    ...
    try:
        ...
        start = datetime.now()
    duration = client_data[message.chat.id]["sub1_duration"]
...
def collect_second_subscription(message):
    ...
    client_data[message.chat.id]["sub1_start"] = sub1_start.strftime("%d.%m.%Y")
    client_data[message.chat.id]["sub1_end"] = sub1_end.strftime("%d.%m.%Y")
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    markup.add("EA Play")
...
def collect_second_duration(message):
    remember_message(message)
    client_data[message.chat.id]["sub2_type"] = message.text
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
...
def collect_games(message):
    ...
    if message.text == "Отмена":
        ...
        return bot.send_message(message.chat.id, "Добавление отменено.", reply_markup=main_keyboard())
    if message.text == "Нет":
        client_data[message.chat.id]["games"] = ""
        finish_add(message)
    else:
...
def save_games(message):
    remember_message(message)
    client_data[message.chat.id]["games"] = " —— ".join(message.text.strip().split('\n'))
    finish_add(message)

def finish_add(message):
    data = (
        client_data[message.chat.id].get("username", ""),
        client_data[message.chat.id].get("birth_date", ""),
        client_data[message.chat.id].get("email", ""),
        client_data[message.chat.id].get("account_password", ""),
        client_data[message.chat.id].get("mail_password", ""),
        client_data[message.chat.id].get("subscription_name", "не оформлена"),
        client_data[message.chat.id].get("subscription_start", ""),
        client_data[message.chat.id].get("subscription_end", ""),
        client_data[message.chat.id].get("region", ""),
        client_data[message.chat.id].get("games", ""),
        client_data[message.chat.id].get("reserve_photo", None)
    )
    add_client(data)
    full_clear(message.chat.id)
